...
记 raw observation concat 张量

$\tilde{o} = \text{concat}(\text{R3M}(I^\text{agent}_{t-1}),\, \text{R3M}(I^\text{wrist}_{t-1}),\, q_{t-1},\, g_{t-1},\, \text{R3M}(I^\text{agent}_t),\, \text{R3M}(I^\text{wrist}_t),\, q_t,\, g_t) \in \mathbb{R}^{\tilde{d}_o}$

其中 $q_t \in \mathbb{R}^7$ 关节角，$g_t \in \mathbb{R}^2$ gripper 状态，$T_o = 2$ 历史帧。$\tilde{d}_o = 2 \times (4096 + 7 + 2) = 8210$。不含 ee_states，与 DP LIBERO 标尺一致。

§3.2 中 condition 路径的观测向量 $o$ 定义为 ObsEncoder 输出：

$o = \text{ObsEncoder}(\tilde{o}) \in \mathbb{R}^{d_o}, \quad d_o = 8 \cdot \text{dim} = 2048$

ObsEncoder（`src/model/ObsEncoder.py`）将 $\tilde{o}$ 按 8 个 (modality, step) token 切分（cam_agent / cam_wrist / proprio / gripper $\times$ $\{t-1, t\}$），per-modality 投影到 $\mathbb{R}^{\text{dim}}$ 后加 learnable step 与 modality position embedding，过 PreNorm Transformer block + final LayerNorm，flatten 8 token 为 $(B, 8 \cdot \text{dim})$。具体超参（dim、num_blocks、num_heads、mlp_ratio）、token 顺序与跨步权重共享策略见 `notes/decisions.md`。
...
- Phase 1（loss 监控）：每 epoch 末更新 `loss_ema_t = 0.95 * loss_ema_{t-1} + 0.05 * loss_t`。
  当 `t >= 20` 且 `(loss_ema[t-20] - loss_ema[t]) / loss_ema[t-20] < 0.02` 时进入 Phase 2。
  Phase 1 期间不调用 evaluate，不保存 best checkpoint（仅每 epoch 覆盖 last）。
- Phase 2（SR 监控）：每 20 epoch 调用一次 evaluate（`n_episodes = 20`），SR 连续两次未上升（`sr >= best_sr + 0.05` 算上升, margin 入 `cfg.early_stop.sr_uplift_margin`；decisions 2026-05-19 修订 row 23 决策 2）即停（patience = 2）。
  best checkpoint 按 EMA 权重的 evaluate 输出 SR 选定。
- 退出条件：SR 早停触发，或 epoch 达到 200 上限。
...
单一 `evaluate(model, obs_encoder, task_name, seed, n_episodes, ...)` 函数。下述三种用法仅参数与调用上下文不同：

- **训练中 evaluate**（早停判定）：`n_episodes = 20`，固定起始状态种子，每 20 epoch 一次。
- **训练后 evaluate**（训练结束）：`n_episodes = 20`，固定起始状态种子，独立于训练中 evaluate 种子。
- **ODE ablation evaluate**：每个训练好的 CFM checkpoint 在 $N \in \{1, 2, 4, 8, 16\}$ 上各跑 20 episodes。
...
│   ├── model/
│   │   ├── modules.py      # FiLMResBlock1D, Down/Upsample1d, sinusoidal_embedding
│   │   ├── unet1d.py       # ConditionalUnet1D (CFM/DP 共享 backbone)
│   │   └── ObsEncoder.py   # ObsEncoder + PreNormBlock (8210 → 2048)
│   ├── cfm.py           # CFM loss, Euler ODE solver
│   ├── dp.py            # DDPM loss, DDIM sampler
│   ├── train.py         # Training entry, EMA, early stopping
│   ├── eval.py          # 训练后 evaluate, video saving
...
FiLM ResBlock（§3.2）→ `src/model/modules.py::FiLMResBlock1D`
